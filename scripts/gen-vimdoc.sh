#!/usr/bin/env bash
#
# Generate doc/bemtvi-keys-helper.txt (the vimdoc help file) from
# doc/bemtvi-keys-helper.md using panvimdoc <https://github.com/kdheepak/panvimdoc>
# (MIT, Dheepak Krishnamurthy). panvimdoc drives pandoc to do all the vimdoc column
# math — right-aligned *tags*, the table of contents, and tw=78 reflow — so the help
# file is never hand-aligned. Edit doc/bemtvi-keys-helper.md, then run this.
#
...
# --- per-plugin settings -----------------------------------------------------
PROJECT="bemtvi-keys-helper"                # → :help bemtvi-keys-helper
DESCRIPTION="A which-key popup for bemtvi"  # header tagline (line 1)
# -----------------------------------------------------------------------------
